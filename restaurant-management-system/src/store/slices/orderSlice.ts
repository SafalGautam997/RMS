...
const orderSlice = createSlice({
  ...
  reducers: {
    ...
    addItemToCurrentOrder: (state, action: PayloadAction<{ menuItem: any; quantity: number }>) => {
      const existingItemIndex = state.currentOrder.items.findIndex(
        item => item.menu_item_id === action.payload.menuItem.id
      );
      ...
      state.currentOrder.totalPrice = state.currentOrder.subtotal - state.currentOrder.discountAmount;
    },
    ...
    updateItemQuantity: (state, action: PayloadAction<{ menuItemId: number; quantity: number }>) => {
      const item = state.currentOrder.items.find(
        item => item.menu_item_id === action.payload.menuItemId
      );
      if (item) {
    ...
    applyDiscount: (state, action: PayloadAction<{ type: 'Percentage' | 'Fixed'; value: number }>) => {
      if (action.payload.type === 'Percentage') {
        state.currentOrder.discountAmount = (state.currentOrder.subtotal * action.payload.value) / 100;
      } else {
